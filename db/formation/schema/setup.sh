...
USER_SQL=$(cat << HERE
CREATE USER ${DB_FORMATION_NAME}_dba PASSWORD '${DB_FORMATION_DBA_PASSWORD}';
...
echo $SETUP_SQL | psql -X -e -d ${DB_FORMATION_SUPER_URL_NO_DBNAME}${DB_FORMATION_NAME} -f -
echo $SETUP_SQL | psql -X -e -d ${DB_FORMATION_SUPER_URL_NO_DBNAME}${DB_FORMATION_NAME}_test -f -
